...
class anaer:
    def __init__(self, config, DK = 1, largescale = 0, pred = 1, patches = 0):
        self.config = config
    ...
    def pred_3tasks(self):
        self.heights_pr, self.heights_gt, _ = predict_segcount_chm(self.config, self.all_files, self.models, self.model_chm)
    ...
    def polyonize_FI(self, DK = 0):
        ...
        # # with heights
        # polygons_dir = os.path.join(self.config.output_dir, "polygons")
        # create_polygons_heights(self.config.output_dir, polygons_dir, self.config.output_dir, postproc_gridsize = (10, 10), postproc_workers = 20, DK = DK)
    ...
    def segcount_CN(self,  inf = 1):
        predict_segcount_cn(self.config, self.all_files, self.models, self.model_chm, self.config.output_dir, eva = 0, inf = inf)
        return 
...
def load_files(config):
    exclude = set(['water_new', 'md5', 'pred'])  
    all_files = []
...
#         with rasterio.open(fullPath) as img:
#             detectedMaskSeg, detectedMaskDens, detectedMeta = detect_tree_segcount_fi(config, model_segcount, img, width = config.WIDTH, height = config.HEIGHT, stride = config.STRIDE, normalize=True, 
#                                                                                         auxData = config.aux_data, singleRaster=config.single_raster, multires = config.multires, upsample = config.upsample, downsave = config.downsave)
#             writeMaskToDisk(detectedMaskSeg, detectedMeta, outputFile,  image_type = config.output_image_type, output_shapefile_type = config.output_shapefile_type, write_as_type = config.output_dtype, th = th, create_countors = False)
...
#                     counter += 1
#             except: # patches
#                 chmPath = os.path.join(config.input_image_dir, Path(filename).stem + '_chm.tif')
#                 # print(chmPath)
#                 with rasterio.open(chmPath) as chm:
#                     if config.upsample and config.downsave:
#                         # detectedMask, detectedMeta = detect_tree_rawtif_fi(config, model, img, config.channels, q1, q3, mins, maxs, width = config.WIDTH, height = config.HEIGHT, stride = config.STRIDE, normalize=config.normalize, maxnorm = config.maxnorm, upsample = config.upsample, downsave = config.downsave) # WIDTH and HEIGHT should be the same and in this case Stride is 50 % width
...
# def predict_using_model_segcount_fi(model, batch, batch_pos, maskseg, maskdens, operator):
#     # batch: [[(256, 256, 5), (128, 128, 1)], [(256, 256, 5), (128, 128, 1)], ...]. len = 200
#     b1 = batch[0]
#     if len(b1) == 2: # 2 inputs
#         # print('2 inputs')
...
def predict_using_model_chm_fi(model, batch, batch_pos, mask, operator, upsample = 1, downsave = 1):
    # batch: [[(256, 256, 5), (128, 128, 1)], [(256, 256, 5), (128, 128, 1)], ...]. len = 200
...
def detect_tree_segcount_fi(config, models, img, width=256, height=256, stride = 128, normalize=True, auxData = 0, singleRaster = 1, multires = 1, upsample = 1, downsave = 1, upscale = 2, rescale_values = 1, inf = 1):
    # for fi 3 bands
    ...
    for col_off, row_off in tqdm(offsets):
        ...
        if normalize:
            ...
            if CHM and multires:
                ...
                ##TODO
                # temp_im2 = image_normalize(temp_im2, axis=(0,1))
        elif config.segcount_tilenorm:
            # print('tile norm', means)
...
def detect_tree_rawtif_fi(config, model, img, channels,  width=256, height=256, stride = 128, normalize=0, maxnorm = 0, upsample = 1, downsave = 0):
...
#     print(nrows, nols)
    ...
    for col_off, row_off in tqdm(offsets):
        ...
        # batch_pos.append((int(window.col_off/2), int(window.row_off/2), int(window.width/2), int(window.height/2)))
        if (len(batch) == config.BATCH_SIZE):
            mask = predict_using_model_chm_fi(model, batch, batch_pos, mask, config.operator)
            batch = []
            batch_pos = []
...
def predict_segcount_cn(config, all_files, model_segcount, model_chm, output_dir, eva = 0, inf = 1):
    counter = 1
    th = 0.5
    counts = {}
    if eva:
    ...
    for fullPath, filename in tqdm(all_files):
        ...
        if not os.path.exists(outputFile):
            ...
            with rasterio.open(fullPath) as img:
                ...
                detectedMaskSeg, detectedMaskDens, detectedMeta = detect_tree_segcount_fi(config, model_segcount, img, width = config.WIDTH, height = config.HEIGHT, stride = config.STRIDE, normalize=config.normalize, 
                                                                                            auxData = config.aux_data, singleRaster=config.single_raster, multires = config.multires, upsample = config.upsample, downsave = config.downsave, upscale = config.upscale, rescale_values=config.rescale_values, inf = inf)
                writeMaskToDisk(detectedMaskSeg, detectedMeta, outputFile,  image_type = config.output_image_type, output_shapefile_type = config.output_shapefile_type, write_as_type = config.output_dtype, th = th, create_countors = False)
...
def writeMaskToDisk(detected_mask, detected_meta, wp, image_type, output_shapefile_type, write_as_type = 'uint8', th = 0.5, create_countors = False, convert = 1):
