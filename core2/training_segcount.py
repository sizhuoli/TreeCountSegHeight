...
class trainer:
    ...
    def train_config(self):
        ...
        model_path = os.path.join(self.config.model_path,'trees_{}_{}_{}_{}_{}_frames_{}_{}_{}weight_{}{}.h5'.format(
                        timestr, self.config.OPTIMIZER_NAME, self.config.chs, self.config.input_shape[0], self.no_frames, self.config.LOSS_NAME, 
                        self.config.LOSS2, self.config.boundary_weights, self.config.model_name, self.config.sufix))
    ...
    def LOAD_model(self):
        ...
        if 'complex' in self.config.model_name:
            ...
            self.model = UNet([self.config.BATCH_SIZE, *self.config.input_shape],self.config.input_label_channel)
